...
export class SavedPostsAsideComponent implements OnInit, OnDestroy {
  ...
  unsavePost(postId: string) {
    ...
    const unsaveSub = this.userService.unsavePost(this.currentUser.uid, postId)
      .subscribe({
        next: () => {
          this.savedPosts = this.savedPosts.filter(post => post._id !== postId);
          if (this.currentUser) {
            this.currentUser.savedPosts = this.currentUser.savedPosts.filter(id => id !== postId);
          }
        },
